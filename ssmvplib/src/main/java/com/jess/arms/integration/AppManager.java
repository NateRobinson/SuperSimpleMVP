...
public final class AppManager {
    ...
    /**
     * 将在前台的 {@link Activity} 赋值给 {@code currentActivity}, 注意此方法是在  方法执行时将栈顶的 {@link Activity} 赋值给 {@code currentActivity}
     * 所以在栈顶的 {@link Activity} 执行  方法时使用 {@link #getCurrentActivity()} 获取的就不是当前栈顶的 {@link Activity}, 可能是上一个 {@link Activity}
     * 如果在 App 启动第一个 {@link Activity} 执行  方法时使用 {@link #getCurrentActivity()} 则会出现返回为 {@code null} 的情况
    ...
    /**
     * 获取在前台的 {@link Activity} (保证获取到的 {@link Activity} 正处于可见状态, 即未调用 ), 获取的 {@link Activity} 存续时间
     * 是在  之前, 所以如果当此 {@link Activity} 调用  方法之后, 没有其他的 {@link Activity} 回到前台(用户返回桌面或者打开了其他 App 会出现此状况)
